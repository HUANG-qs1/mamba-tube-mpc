# -*- coding: utf-8 -*-
"""fig3_scenes.py - Fig.3 v1: three-scene trajectory comparison (2x2 composite)"""
import os
import numpy as np
...
d = np.load("fig3_trajectories.npz", allow_pickle=True)

def load(sc):
    ref = d[f"{sc}_ref"]
    sd = int(d[f"{sc}_seed"])
    errs = {"ours": d[f"{sc}_ours_err"], "standard": d[f"{sc}_std_err"], "ekf": d[f"{sc}_ekf_err"]}
    return ref, sd, errs
...
PANELS = [
    (axa, "scene1", "(a) Scene 1: circle + constant"),
    (axb, "scene2", "(b) Scene 2: figure-8 + random"),
    (axc, "scene3", "(c) Scene 3: sine + composite"),
]
for ax, sc, tag in PANELS:
    ref, sd, errs = load(sc)
    ...
    ax.set_title(tag, fontsize=8, loc="left", pad=3)
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ax.set_aspect("equal", adjustable="datalim")
    ax.tick_params(labelsize=6.5)

# ---- (d) position error vs time: scene 2 (post-warmup) ----
ref, sd, errs = load("scene2")
t = np.arange(len(ref)) * 0.1
for m in ["standard", "ekf", "ours"]:
    en = np.hypot(errs[m][:, 0], errs[m][:, 1])
    axd.plot(t[200:], en[200:], **style(m))
...
axd.set_title("(d) position error - scene 2 (post-warmup)", fontsize=8, loc="left", pad=3)
...
ps.save_fig(fig, "figures/fig3_scenes")
